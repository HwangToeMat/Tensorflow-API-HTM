...
def find_id_by_name(class_label_df, target_name):
    return class_label_df.loc[class_label_df['Category'] == target_name]['LabelName'].values[0]
...
def create_tf_record(output_filename,
    ...
    with contextlib2.ExitStack() as tf_record_close_stack:
        ...
        for idx, example in enumerate(examples):
            ...
            data = {}
            object_list = []
            matched_rows = annotation_df.loc[annotation_df['OriginalURL'].map(
                lambda x: x.split('/')[-1].replace('.jpg', '')) == example]
...
def main(_):
    ...
    class_label_df = pd.read_csv(CLASS_FILE_PATH)
    print(class_label_df.head())

    target_class_id_dict = {}
    for class_name in TARGET_CLASSES:
        target_class_id_dict[class_name] = find_id_by_name(
            class_label_df, class_name)
    ...
    annotation_df = pd.read_csv(ANNOTATION_FILE_PATH, header=0)
    ...
    train_output_path = os.path.join(
        FLAGS.output_dir, 'Football_class_train.record')
    val_output_path = os.path.join(
        FLAGS.output_dir, 'Football_class_val.record')
    create_tf_record(
        train_output_path,
        FLAGS.num_shards,
        label_map_dict,
